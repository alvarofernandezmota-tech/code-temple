...
def check_documentation(folder):
    ...
    for root, dirs, files in os.walk(folder):
        ...
        # Verifica si hay README.md en cada carpeta
        if 'README.md' not in files and os.path.basename(root) not in ['python', 'scripts', 'scriptscd']:
            if os.path.exists(os.path.join(root, 'README.md')) == False:
...
def main():
    ...
    madre_path = os.path.dirname(os.path.abspath(__file__))
    madre_path = os.path.join(madre_path, '..')
    os.chdir(madre_path)
    
    print(f"\nDirectorio: {os.getcwd()}")
    
    # Define carpetas válidas
    valid_folders = [
